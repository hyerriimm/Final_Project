...
export const __detail = createAsyncThunk(
    ...
    async (payload, thunkAPI) => { 
        try {
            const data = await axios.get(`http://13.209.47.67/post/detail/${payload}`, {
            headers: {
...
//게시글 삭제
  export const __delete = createAsyncThunk(
    ...
    async (payload, thunkAPI) => { 
        try {
            const data = await axios.delete(`http://13.209.47.67/post/${payload}`, {
              headers: {
                authorization: localStorage.getItem("ACCESSTOKEN"),
                refreshtoken: localStorage.getItem("REFRESHTOKEN")
              }
...
export const __addWish = createAsyncThunk(
  ...
  async (payload, thunkAPI) => { 
      try {
          const data = await axios.post(`http://13.209.47.67/post/add/wish/${payload}`, payload, {
            headers: {
              authorization: localStorage.getItem("ACCESSTOKEN"),
              refreshtoken: localStorage.getItem("REFRESHTOKEN")
            }
...
export const __removeWish = createAsyncThunk(
  ...
  async (payload, thunkAPI) => { 
      try {
          const data = await axios.post(`http://13.209.47.67/post/remove/wish/${payload}`, payload, {
            headers: {
              authorization: localStorage.getItem("ACCESSTOKEN"),
              refreshtoken: localStorage.getItem("REFRESHTOKEN")
            }
